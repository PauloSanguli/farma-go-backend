...
def upgrade() -> None:
    ...
    op.add_column('medicine', sa.Column('stock_id', sqlmodel.sql.sqltypes.AutoString(), nullable=False))
    ...
    op.alter_column('pharmacist', 'password',
               existing_type=sa.VARCHAR(length=255),
               type_=sqlmodel.sql.sqltypes.AutoString(length=500),
               existing_nullable=False)
    op.alter_column('pharmacist', 'email',
               existing_type=sa.VARCHAR(length=100),
               type_=sqlmodel.sql.sqltypes.AutoString(length=120),
               existing_nullable=False)
    op.add_column('pharmacy', sa.Column('stock_id', sqlmodel.sql.sqltypes.AutoString(), nullable=True))
    op.create_foreign_key(None, 'pharmacy', 'stock', ['stock_id'], ['id'])
    op.alter_column('pharmacyimage', 'image_url',
               existing_type=sa.VARCHAR(length=255),
               type_=sqlmodel.sql.sqltypes.AutoString(length=500),
               existing_nullable=False)
...
def downgrade() -> None:
    ...
    op.alter_column('pharmacist', 'password',
               ...
               type_=sa.VARCHAR(length=255),
               existing_nullable=False)
    op.add_column('medicine', sa.Column('pharmacy_id', sa.VARCHAR(), autoincrement=False, nullable=False))
    op.add_column('medicine', sa.Column('stock', sa.INTEGER(), autoincrement=False, nullable=False))
    op.drop_constraint(None, 'medicine', type_='foreignkey')
    op.create_foreign_key('medicine_pharmacy_id_fkey', 'medicine', 'pharmacy', ['pharmacy_id'], ['id'])
